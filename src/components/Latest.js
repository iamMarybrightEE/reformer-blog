 ...
 const {  latest, setLatest } = useStateContext();
  ...
  return (
    <div className="mt-8  w-full p-4">
      {!(latest) && [1,2,3,4,5].map((n) => <SkeletonArticle key={n} theme="light" />)}
               {latest && <>
        <motion.div variants={containerVariants}
          ...
          exit="exit"   className="pb-8 w-full  overflow-hidden text-justify   transition-all relative ">
            ...
            <div className="text-justify">
               ...
               {latest && <div className=" mt-16 pb-4 w-full  dark:text-gray-200  ">
        <motion.div variants={containerVariants}
          initial="down"
          animate="up"
      ...
      {latest.filter((item,i) => (i !== 0 )).map((item) => (
                  <>
          ...
          exit="exit" className="sm:flex gap-4 ">
                      <div className=" relative">
                        ...
                        <div className={item.file == null ? "hidden " : ' absolute top-5 left-5 mb-2 p-2 py-1 font-bold bg-[#4432cd]  text-white w-[max-content] rounded-md'}>  latest</div>
                      </div>
